// COPIED VERBATIM from src/lib/zone.js. Do not edit this file directly.
//
// WHY A COPY. The editor is a separate static deployment with no build step --
// deliberately, so that what ships is what you can read. It therefore cannot
// import across the repo, and this is the one piece of the public site's logic
// it genuinely needs: turning a wall-clock time typed into a form into the UTC
// instant `events.starts_at` stores, correctly across a DST changeover.
//
// Reimplementing it would have been the worse choice. The two-pass offset
// correction below is not obvious, it is already covered by
// tests/occurrences.test.js, and a second slightly-different version would
// disagree with the site about what "21:00" means on exactly two nights a year.
//
// THE COPY IS HELD HONEST BY A TEST. tests/zone-copy.test.js imports BOTH this
// file and src/lib/zone.js and asserts they agree, including across the spring
// and autumn changeovers. If the original changes and this does not, that test
// fails -- which is the only reason a duplicate like this is tolerable.
